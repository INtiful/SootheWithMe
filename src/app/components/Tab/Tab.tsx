...
const Tab = ({ type, isActive, onClick }: TabProps) => {
  ...
  return (
    <div
      className={`relative flex items-center gap-4 overflow-hidden px-4 pb-4 text-18 font-semibold transition ${styles} before:absolute before:bottom-0 before:w-full before:border-b-2 before:border-var-gray-900 before:transition-all before:duration-300`}
      onClick={onClick}
    >
      {type === 'workation' ? (
        <>
